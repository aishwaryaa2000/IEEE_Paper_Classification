...
<img class="wave" src="../Assets/img/wave2.png">
	<div class="container">
		<div class="img">
			<img src="../Assets/SVG/pochange.svg">
		</div>
		<div class="login-content">
			<form method="POST">
				<img src="../Assets/SVG/po_forgot_man.svg">
  ...
  if(myInput.value.match(lowerCaseLetters) && myInput.value.match(upperCaseLetters) && myInput.value.match(numbers) && myInput.value.match(spl) && (myInput.value.length >= 8)) {  
    ...
    document.getElementById("messageg").style.display = "none";
     ...
     <br>     	<br>
            	<center><input type="submit" class="btn" value="Update password" name="change"></center>
            </form>
        </div>
    </div>
...
function checkPass()
{
	...
	{
		document.getElementById('confirm-message2').innerHTML= " ";
	}
...
<input type="submit" name="change" value="Update Password">
    ...
    if(@isset($_POST['change']))
    {
        ...
        $query_check="SELECT * FROM `po_login_details` WHERE `po_user_id`='$po_user_id' and `po_password`='$old_password'";
        $res_check=mysqli_query($conn,$query_check);
        if(mysqli_num_rows($res_check)>0)
        {
            if($new==$confirm)
            {
                $query_update="UPDATE `po_login_details` SET `po_password`='$new_password' WHERE `po_user_id`='$po_user_id'";
