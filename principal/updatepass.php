...
<input type="submit" name="change" value="Update Password">
    ...
    if(@isset($_POST['change']))
    {
        ...
        $query_check="SELECT * FROM `principal_login_details` WHERE `principal_user_id`='$principal_user_id' and `password`='$old_password'";
        $res_check=mysqli_query($conn,$query_check);
        if(mysqli_num_rows($res_check)>0)
        {
            if($new==$confirm)
            {
                $query_update="UPDATE `principal_login_details` SET `password`='$new_password' WHERE `principal_user_id`='$principal_user_id'";
