...
<head>
    ...
    <title>Contact Details of the college</title>
...
<div class="jumbotron">
    <ul class="navbar font-weight-bold mynav2">
              <li class="nav-item list-unstyled col-sm-4 md-4 xl-6">
                <div class="nav-link text-white text-center mylabels">Contact Details of the college</div>
              </li>
              <li class="nav-item  list-unstyled col-sm-4 md-4 xl-6">
                <div class="nav-link text-white text-center  my_buttons"></div>
              </li>
        </ul>
    <div class="card">
    ...
    <th><b>Email id </b></th>
    <th><b>Grant in aid status</b></th>
    <th><b>Minority status</b></th>
    <th><b>If minority college:Linguistic/Religious</b></th>
       </tr>
    </thead>
    <tbody>
    <?php
...
if (mysqli_num_rows($result) > 0){
     while ($data = mysqli_fetch_assoc($result)) {
        ...
        echo '<td>'.$data['college_address'].'</td>';
        echo '<td>'.$data['college_telephone'].'</td>';
        echo '<td>'.$data['college_email'].'</td>';
        echo '<td>'.$data['college_status_grant'].'</td>';
        echo '<td>'.$data['college_status_minority'].'</td>';
        echo '<td>'.$data['college_pl_state'].'</td>';
        echo '</tr>';
        $srno += 1;
        }
}
