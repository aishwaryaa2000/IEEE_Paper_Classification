...
<head>
    ...
    <title>Adopted area details for Special Camping Activities</title>
    <style>
        /* table,th,td{ 
...
<div class="jumbotron">
    <ul class="navbar font-weight-bold mynav2">
              <li class="nav-item list-unstyled col-sm-4 md-4 xl-6">
                <div class="nav-link text-white text-center mylabels">Adopted area details for Special Camping Activities</div>
              </li>
              <li class="nav-item  list-unstyled col-sm-4 md-4 xl-6">
...
if (mysqli_num_rows($result) > 0){
     while ($data = mysqli_fetch_assoc($result)) {
        $add = $data['sba_village_camp'].','.$data['sba_atpost_camp'].','.$data['sba_taluka_camp'].','.$data['sba_district_camp'];
        echo '<tr>';       
        echo '<td>'.$srno.'</td>';
        echo '<td>'.$data['college_code'].'</td>';
        echo '<td>'.$data['college_name'].'</td>';
        echo '<td>'.$add.'</td>';
        echo '<td>'.$data['sba_per_name_camp'].'</td>';
        echo '<td>'.$data['sba_per_contact_camp'].'</td>';
        echo '</tr>';
        $srno += 1;
        }
}
