...
<head>
    ...
    <title>Details of last year’s Regular Activities</title>
    <style>
        /* table,th,td{ 
...
<div class="jumbotron">
    <ul class="navbar font-weight-bold mynav2">
              <li class="nav-item list-unstyled col-sm-4 md-4 xl-6">
                <div class="nav-link text-white text-center mylabels">Details of last year’s Regular Activities</div>
              </li>
              <li class="nav-item  list-unstyled col-sm-4 md-4 xl-6">
    ...
    <th><b>Sr.No.</b></th>
        ...
        <th><b>Name and address of the adopted area/village </b></th>
        <th><b>No of students who completed 120 hours</b></th>
        <th><b>No of students who completed 240 hours</b></th>
        <th><b> Status of submission of audited statement of accounts YES/NO</b></th>
       </tr>
    </thead>
    <tbody>
    <?php
...
if (mysqli_num_rows($result) > 0){
     while ($data = mysqli_fetch_assoc($result)) {
        echo '<tr>';       
        echo '<td>'.$data['sr_no'].'</td>';
            echo '<td>'.$data['college_code'].'</td>';
            echo '<td>'.$data['college_name'].'</td>';
            echo '<td>'.$data['prev_adopted_village'].'</td>';
            echo '<td>'.$data['prev_student_120hrs'].'</td>';
            echo '<td>'.$data['prev_student_240hrs'].'</td>';
            echo '<td>'.$data['prev_audit_adopt'].'</td>';
        echo '</tr>';
        $srno += 1;
        }
}
