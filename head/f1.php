...
<head>
    ...
    <title>Adopted area details for Regular Activities</title>
    <style>
        /* table,th,td{ 
...
<div class="jumbotron">
    <ul class="navbar font-weight-bold mynav2">
              <li class="nav-item list-unstyled col-sm-4 md-4 xl-6">
                <div class="nav-link text-white text-center mylabels">Adopted area details for Regular Activities</div>
              </li>
              <li class="nav-item  list-unstyled col-sm-4 md-4 xl-6">
    ...
    <th><b>Exact address of the adopted area</b></th>
    <th><b>Name of the local contact person</b></th>
    ...
    <?php
include '../connect.php';
$qu = "SELECT * FROM `adopted_area` ORDER BY `college_code` ASC";
...
if (mysqli_num_rows($result) > 0){
     while ($data = mysqli_fetch_assoc($result)) {
        if(!$data['area_address']==''&&mysqli_fetch_row(mysqli_query($conn,"SELECT `verification` FROM `performa_1_reg` WHERE `college_code` = '{$data['college_code']}' "))[0]){
        echo '<tr>';       
        echo '<td>'.$srno.'</td>';
        echo '<td>'.$data['college_code'].'</td>';
        echo '<td>'.$data['college_name'].'</td>';
        echo '<td>'.$data['area_address'].'</td>';
        echo '<td>'.$data['area_local_person'].'</td>';
        echo '<td>'.$data['area_local_contact'].'</td>';
        echo '</tr>';
        $srno += 1;
        }
    }
