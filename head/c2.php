...
<head>
    ...
    <title>Other information about the program officer</title>
    <style>
        /* table,th,td{ 
...
<div class="jumbotron">
    <ul class="navbar font-weight-bold mynav2">
              <li class="nav-item list-unstyled col-sm-4 md-4 xl-6">
                <div class="nav-link text-white text-center mylabels">Other information about the program officer</div>
              </li>
              <li class="nav-item  list-unstyled col-sm-4 md-4 xl-6">
    ...
    <th><b>Sr.No.</b></th>
        ...
        <th><b>Date of appointment as NSS Program Officer</b></th>
        <th><b>Status of NSS Orientation YES/NO</b></th>
        <th><b>Year of NSS orientation if completed</b></th>
       </tr>
    </thead>
    <tbody>
    <?php
include '../connect.php';
$qu = "SELECT * FROM `po_details`";
...
if (mysqli_num_rows($result) > 0){
     while ($data = mysqli_fetch_assoc($result)) {
         if(!$data['po_name']==''&&mysqli_fetch_row(mysqli_query($conn,"SELECT `verification` FROM `performa_1_reg` WHERE `college_code` = '{$data['college_code']}' "))[0]){
            ...
            echo '<td>'.$data['date_appointment_po'].'</td>';
            echo '<td>'.$data['nss_orientation'].'</td>';
