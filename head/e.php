...
<head>
    ...
    <title>Details of the student leaders for the academic year</title>
    <style>
        /* table,th,td{ 
...
<div class="jumbotron">
    <ul class="navbar font-weight-bold mynav2">
              <li class="nav-item list-unstyled col-sm-4 md-4 xl-6">
                <div class="nav-link text-white text-center mylabels">Details of the student leaders for the academic year</div>
              </li>
              <li class="nav-item  list-unstyled col-sm-4 md-4 xl-6">
    ...
    <th><b>Name of the student leader</b></th>
    <th><b>Class</b></th>
    <th><b>Mobile No</b></th>
    <th><b>Email id</b></th>
       </tr>
    ...
    <?php
include '../connect.php';
$qu = "SELECT * FROM `student_leader` ORDER BY `college_code` ASC";
...
if (mysqli_num_rows($result) > 0){
     while ($data = mysqli_fetch_assoc($result)) {
        if(!$data['leader_name']==''&&mysqli_fetch_row(mysqli_query($conn,"SELECT `verification` FROM `performa_1_reg` WHERE `college_code` = '{$data['college_code']}' "))[0]){
        echo '<tr>';       
        echo '<td>'.$srno.'</td>';
        echo '<td>'.$data['college_code'].'</td>';
        echo '<td>'.$data['college_name'].'</td>';
        echo '<td>'.$data['leader_name'].'</td>';
        echo '<td>'.$data['class'].'</td>';
        echo '<td>'.$data['leader_mobile'].'</td>';
        echo '<td>'.$data['leader_email'].'</td>';
        echo '</tr>';
        $srno += 1;
        }
    }
