...
<head>
    ...
    <title>Details of last year’s Special Camping Activities</title>
    <style>
        /* table,th,td{ 
...
<div class="jumbotron">
    <ul class="navbar font-weight-bold mynav2">
              <li class="nav-item list-unstyled col-sm-4 md-4 xl-6">
                <div class="nav-link text-white text-center mylabels">Details of last year’s Special Camping Activities</div>
              </li>
              <li class="nav-item  list-unstyled col-sm-4 md-4 xl-6">
    ...
    <th rowspan="2"><b>Sr.No.</b></th>
        <th rowspan="2"><b>File No.</b></th>
        <th rowspan="2"><b>Name of the College</b></th>
        <th rowspan="2"><b>Name and address of the camp location</b></th>
        <th colspan="2"><b>Dates of special camp</b></th>
        <th rowspan="2"><b>No of students who attended the special camp </b></th>
        <th rowspan="2"><b> Status of submission of audited statement of accounts YES/NO</b></th>
       </tr>
       </tr>
         <tr>
             <th><b>From</b></th>
    ...
    <?php
include '../connect.php';
$qu = "SELECT * FROM `performa_1_reg` WHERE `verification` = 1";
...
if (mysqli_num_rows($result) > 0){
     while ($data = mysqli_fetch_assoc($result)) {
        $add = $data['prev_venue_camp'].','.$data['prev_post_camp'].','.$data['prev_taluka_camp'].','.$data['prev_district_camp'];
        echo '<tr>';       
        echo '<td>'.$srno.'</td>';
            echo '<td>'.$data['college_code'].'</td>';
            echo '<td>'.$data['college_name'].'</td>';
            echo '<td>'.$add.'</td>';
            echo '<td>'.$data['prev_starting_date_camp'].'</td>';
            echo '<td>'.$data['prev_ending_date_camp'].'</td>';
            echo '<td>'.$data['prev_volunteer_camp'].'</td>';
            echo '<td>'.$data['prev_audit_camp'].'</td>';
        echo '</tr>';
        $srno += 1;
        }
}
